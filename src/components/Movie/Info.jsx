  ...
  return (
    <div>
      <div className='md:flex md:gap-12 items-start font-["Inter"]'>
        ...
        <div className='md:h-[576px] text-white md:flex md:flex-col gap-5 mt-2 md:mt-0'>
          <div>
            <h1 className='md:text-4xl md:max-w-[800px] text-[20px]'>
              {movie.title}
            </h1>
            {movie.tagline && (
              <p className='italic text-cyan-200 text-[15px] md:text-lg md:max-w-[800px]'>
                {movie.tagline}
              </p>
            )}
          </div>
            ...
            {movie.runtime ? (
              ...
              <p className='text-white text-sm md:text-xl'>
                ...
                {lang
                  ...
                  : movie.number_of_seasons === 1
                  ? 'сезон '
                  : movie.number_of_seasons > 1 && movie.number_of_seasons <= 4
                  ? 'сезона '
                  : 'сезонов '}{' '}
          ...
          <div className='flex justify-between md:items-baseline md:max-w-[600px] text-white md:text-3xl'>
            <div>
              ...
              <div className='flex gap-2 text-white flex-wrap'>
                {movie?.genres?.map((el) => (
                  <p className='text-[12px] md:text-sm'>{el.name}</p>
                ))}
              </div>
            </div>
            <div>
